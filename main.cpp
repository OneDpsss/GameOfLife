...
#define sizeC 40
#define sizeM 200
#define monkey_age 400
#define banana_age 100
#define croc_age 1000
...
bool sex_croc(int xc, int yc, int id)
{
    for (int _ = 0; _ < 20; _++)
    {
        int i = (rand() % 2 ? 1 : -1) * (rand() % 4);
        int j = (rand() % 2 ? 1 : -1) * (rand() % 4);
        int newx = (xc + i + size) % size;
        int newy = (yc + j + size) % size;
        if (field[newx][newy] == 'c' && newx != xc && newy != yc && C[findC(newy, newy)].age >= 350 &&
...
void croc_action(int ic)
{
    ...
    if (C[ic].satiety > 8 && C[ic].age >= 350)
    {
